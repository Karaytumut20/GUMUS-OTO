...
export function getWebSiteSchema() {
  return {
    ...
    inLanguage: 'tr-TR',
    publisher: { '@id': `${businessConfig.siteUrl}/#business` }
  };
}
...
export function getServiceSchema({ name, description, url, areaServed }: {
  name: string;
  description: string;
  url: string;
  areaServed?: string;
  ...
  return {
    ...
    areaServed: {
      '@type': 'AdministrativeArea',
      name: areaServed || 'Ümraniye ve Ataşehir, İstanbul'
    }
  };
}
